...
export function MetadataPicker({ onChoose, onCameraOpening }: { onChoose: (book: BookLookup) => void; onCameraOpening: () => void }) {
  ...
  return <View style={{ gap: 12, paddingVertical: 16 }}>
    ...
    {results.map((book, index) => <View key={`${book.isbn}-${index}`} style={{ alignItems: 'center', flexDirection: 'row', gap: 10 }}><>{book.cover_url ? <Image accessibilityLabel={`${book.title}封面`} source={{ headers: { Referer: 'https://book.douban.com/' }, uri: book.cover_url }} style={{ backgroundColor: colors.paperBright, borderRadius: 6, height: 54, width: 38 }} /> : <View style={{ backgroundColor: colors.paperBright, borderRadius: 6, height: 54, width: 38 }} />}</><View style={{ flex: 1 }}><ActionButton quiet onPress={() => onChoose(book)}>{book.title} · {book.author || '作者未提供'}{book.publisher ? ` · ${book.publisher}` : ''}</ActionButton></View></View>)}
  </View>
}
