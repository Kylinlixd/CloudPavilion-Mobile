...
export function AddBookScreen({ navigation }: Props) {
  ...
  async function chooseLocalCover(camera: boolean) {
    try {
      ...
      setMessage("正在上传本地封面……");
      const uploaded = await uploadFile<{ cover_url: string }>(
        "/book-metadata/cover/",
        "image",
        result.assets[0],
      );
  ...
  const guardedSave = useMemo(
    () =>
      createSubmissionGuard<BookIntakeResult | undefined, boolean>(
        async (confirmExisting = false) => {
          if (!draft.title.trim()) {
            setMessage("请先填写书名。");
            return;
          }
          ...
          try {
            if (!confirmExisting) {
              const duplicate = await checkBookDuplicate(draft);
              if (duplicate.exists) {
                setSubmitting(false);
                Alert.alert(
                  "已存在藏书",
            ...
            const result = await addBookToFamily(draft, confirmExisting);
  ...
  return (
    ...
    >
      ...
      <MetadataPicker
        ...
        onChoose={({ source: _source, ...metadata }) => {
          setDraft((current) => ({
            ...current,
            ...metadata,
            publish_date: metadata.publish_date || "",
        ...
        {(["manual", "scan"] as const).map((value) => (
          <TouchableOpacity
            ...
            onPress={() => {
              setMode(value);
              setMessage("");
              setScanning(value === "scan");
            }}
            style={{
              alignItems: "center",
              backgroundColor: mode === value ? colors.ink : "transparent",
              borderRadius: 9,
              flex: 1,
              paddingVertical: 12,
            }}
          >
            <Text
              style={{
                color: mode === value ? colors.white : colors.muted,
                fontFamily: typography.body,
                fontSize: 13,
              }}
            >
              {value === "scan" ? "扫码添加" : "手动添加"}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      {mode === "scan" ? (
        <View style={{ marginTop: spacing.lg }}>
          {!permission ? (
            <Text style={{ color: colors.muted }}>正在读取摄像头权限…</Text>
          ) : !permission.granted ? (
            <View
              style={{
                backgroundColor: colors.paperBright,
            ...
            >
              ...
              >
                云阁只用摄像头读取书背面的 ISBN 条码，不会保存照片。
              </Text>
              {permission.canAskAgain ? (
                <ActionButton onPress={() => void requestPermission()}>
                  允许摄像头
                </ActionButton>
              ) : (
                <Text
                ...
                >
                  请在 iPhone 设置中为云阁开启摄像头权限，或改用手动添加。
                </Text>
              )}
              <ActionButton onPress={() => setMode("manual")} quiet>
                改用手动添加
              </ActionButton>
            </View>
          ) : (
            <>
              <View
                style={{ borderRadius: 18, height: 360, overflow: "hidden" }}
              >
                <CameraView
                  barcodeScannerSettings={{ barcodeTypes: ["ean13"] }}
                  onBarcodeScanned={
                    scanning ? (result) => void handleScan(result) : undefined
                  }
                  style={{ flex: 1 }}
                />
                <View
              ...
              >
                {lookupPending
                  ? "正在查询书目信息…"
                  : "把书背面的 978 / 979 条码放入框内"}
              </Text>
              {!scanning && !lookupPending ? (
        ...
        <View style={{ gap: spacing.md, marginTop: spacing.lg }}>
          ...
          <View style={{ flexDirection: "row", gap: spacing.sm }}>
            <View style={{ flex: 1 }}>
              <ActionButton onPress={() => void chooseLocalCover(false)} quiet>
                从相册选择封面
              </ActionButton>
            </View>
            <View style={{ flex: 1 }}>
              <ActionButton onPress={() => void chooseLocalCover(true)} quiet>
                拍照封面
              </ActionButton>
            </View>
          </View>
          ...
          <FormField
            ...
            placeholder="10 位或 13 位 ISBN（可不填）"
            value={draft.isbn}
          />
          <FormField
            label="出版社"
          ...
          <FormField
            label="出版日期"
            onChangeText={(value) => update("publish_date", value)}
