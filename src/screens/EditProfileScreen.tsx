...
export function EditProfileScreen() {
  const navigation = useNavigation<any>(); const [user, setUser] = useState<User | null>(null); const [nickname, setNickname] = useState(""); const [photo, setPhoto] = useState<any>(null); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [offline, setOffline] = useState(false); const [notice, setNotice] = useState("");
  const load = useCallback(async () => { const result = await apiClient.getWithOfflineCache<User>("/auth/me/", "profile"); setUser(result.data); setNickname(result.data.nickname || ""); setOffline(result.offline); }, []);
  ...
  async function save() { if (!nickname.trim()) { setError("用户名称不能为空。"); return; } setBusy(true); setError(""); setNotice(""); try { const result = await apiClient.mutateWithOfflineQueue<User>({ path: "/auth/me/", method: "PATCH", body: { nickname: nickname.trim() }, idempotencyKey: `profile-nickname-${nickname.trim()}` }); if (result.queued) { setUser((current) => current ? { ...current, nickname: nickname.trim() } : current); setPhoto(null); setNotice("当前离线，昵称已保存到本机，联网后自动同步。头像请在联网后上传。"); return; } if (photo) await uploadFile<User>("/auth/me/avatar/", "avatar", photo); await load(); setPhoto(null); Alert.alert("已保存", "个人资料已更新。", [{ text: "完成", onPress: () => navigation.goBack() }]); } catch (caught) { setError(caught instanceof ApiError ? caught.message : "保存失败，请稍后重试。"); } finally { setBusy(false); } }
  ...
  return <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: 120 }} style={{ backgroundColor: colors.paper }} keyboardShouldPersistTaps="handled">
    ...
    {offline ? <Text accessibilityRole="alert" style={{ color: colors.terracotta, fontSize: 12, marginTop: spacing.md }}>当前离线，正在显示上次同步的资料。</Text> : null}{notice ? <Text accessibilityRole="alert" style={{ color: colors.terracotta, fontSize: 12, marginTop: spacing.md }}>{notice}</Text> : null}{error ? <Text accessibilityRole="alert" style={{ color: colors.danger, marginTop: spacing.md }}>{error}</Text> : null}<View style={{ marginTop: spacing.xl }}><ActionButton disabled={busy} loading={busy} onPress={() => void save()}>保存资料</ActionButton></View>
  </ScrollView>;
}
