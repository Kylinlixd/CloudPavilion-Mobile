...
type FamilyValue = {
  ...
  family: Family | null;
  setCurrentFamilyId: (id: string) => Promise<void>;
...
export function FamilyProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void getFamilyId()
      .then(setFamilyIdState)
      .catch(() => setFamilyIdState(null));
  ...
  const setCurrentFamilyId = useCallback(async (id: string) => {
    const value = id.trim();
    if (!value) return;
    await setFamilyId(value);
    setFamilyIdState(value);
    setFamily(null);
  }, []);
  const refreshFamily = useCallback(async () => {
    if (!familyId) return;
    const result = await apiClient.get<Family[] | { results: Family[] }>(
      "/families/",
    );
    const list = Array.isArray(result) ? result : result.results;
    setFamily(
      list.find((item) => String(item.id) === familyId) || list[0] || null,
    );
  }, [familyId]);
  const value = useMemo(
    () => ({ familyId, family, setCurrentFamilyId, refreshFamily }),
    [familyId, family, setCurrentFamilyId, refreshFamily],
  );
  return (
