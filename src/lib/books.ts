...
export async function lookupBookByIsbn(value: string) {
  ...
  return apiClient.get<BookLookup>(
...
export async function addBookToFamily(
  ...
  if (confirmExisting) payload.confirm_existing = "true";
  if (draft.publish_date.trim())
    payload.publish_date = draft.publish_date.trim();
  return apiClient.post<BookIntakeResult>("/book-copies/add-book/", payload);
}

export async function checkBookDuplicate(draft: BookDraft) {
  return apiClient.post<{ exists: boolean; book?: Book; copy_count?: number }>(
    "/book-copies/check-duplicate/",
    {
      title: draft.title.trim(),
      author: draft.author.trim(),
      isbn: draft.isbn.trim(),
...
export function createSubmissionGuard<T, A = void>(
  operation: (argument?: A) => Promise<T>,
) {
  let pending = false;
  return async (argument?: A): Promise<T | undefined> => {
