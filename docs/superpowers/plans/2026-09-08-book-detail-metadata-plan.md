...
- Test: `/Users/leexd/CloudPavilion/catalog/tests/test_services.py`
...
- [ ] **Step 2: 运行测试确认失败**：`python manage.py test catalog.tests.test_services -v 2`。
...
- [ ] **Step 4: 运行测试确认通过**：再次运行服务测试。
- [ ] **Step 5: 提交**：`git add catalog/services.py catalog/tests/test_services.py && git commit -m "fix: enrich existing books with missing metadata"`。
...
- [ ] **Step 1: 写失败测试**：覆盖元信息字段过滤（空出版社/ISBN 不渲染）和窄屏/iPad 的封面宽度计算。
...
- [ ] **Step 1: 写失败测试**：模拟空简介书籍和 metadata 搜索结果，断言只选取首个非空 description，并调用一次 PATCH；已有简介时断言不搜索、不 PATCH。
- [ ] **Step 2: 运行测试确认失败**：`npm test -- --run src/test/BookDetailScreen.test.tsx`。
...
- [ ] **Step 1: 运行后端相关测试**：`python manage.py test catalog.tests.test_metadata catalog.tests.test_reading catalog.tests.test_services -v 2`。
